...
var isOption = function(){
    var index = argv.indexOf('-only');
    if(index!=-1) return argv[index + 1];
};

var isFile = function(arg){
    return arg[0] != '-';
};
...
function listTestNames(tests,option) {
    return tests.filter(function(each){
       return each.match(option);
    });
}
...
function main() {
    ...
    if (testfile) {
        ...
        if(option){
            tests  =listTestNames(tests,option);
            summary.totalTest = tests.length;
